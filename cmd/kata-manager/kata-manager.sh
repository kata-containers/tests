...
typeset -r default_image_dir="/opt/kata/share/kata-containers"
...
typeset -r default_config_file="/opt/kata/share/defaults/kata-containers/${config_file_name}"
	...
	then
		...
		do
			case "$distro" in
				...
				ubuntu) sudo apt-mark unhold "$pkg" &>/dev/null || true ;;
