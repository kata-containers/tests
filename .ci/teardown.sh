...
collect_logs()
{
	...
	local -r metrics_json_results_dir="${GOPATH}/src/${tests_repo}/metrics/results/"
	local -r metrics_json_results_report_dir="${metrics_json_results_dir}/new"
	local -r metrics_report_dir="${GOPATH}/src/${tests_repo}/metrics/report/output/"
	...
	if [ "${log_copy_dest}" ]; then
		...
		if [ "${result_dir}" != "" ];then
			sudo find "${result_dir}" -name '*.tar.gz' -exec mv {} "${log_copy_dest}" \;
		fi

		# Collect data generated by metrics CI
		# If report was not generated, data is created here
		find "${metrics_json_results_dir}" -maxdepth 1 -name '*.json' -exec mv {} "${log_copy_dest}" \;
		# If report was generated files are here
		find "${metrics_json_results_report_dir}" -maxdepth 1 -name '*.json' -exec mv {} "${log_copy_dest}" \;
		find "${metrics_report_dir}" -maxdepth 1 -name '*.pdf' -exec mv {} "${log_copy_dest}" \;
