...
if [ -f /etc/os-release ]; then
  alinux_version=$(grep VERSION_ID /etc/os-release | cut -d '"' -f2)
else
  alinux_version=$(grep VERSION_ID /usr/lib/os-release | cut -d '"' -f2)
fi

[ "$alinux_version" == 3 ] || die "This script is for alinux 3 only"
...
if [ -f "/etc/yum.repos.d/AliYun.repo" ]; then
...
echo "FIXME !!!!  Enable PowerTools repository"
