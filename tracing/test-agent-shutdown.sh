...
default_triple="${arch}-unknown-linux-${libc}"
...
install_socat()
{
	# Local socat supports VSOCK
	socat -V | egrep -q "\<WITH_VSOCK\>" && return 0
	...
	git clone "$repo" && cd socat

	# For this to succeed: it returns 1 on a seemingly successful run ...!?!
	autoreconf -fi || true
	...
	# Check that this version supports VSOCK
	socat -V |\
		egrep -q "\<WITH_VSOCK\>" \
		|| die "Exiting as need version of socat with VSOCK support"
}
...
setup()
{
	...
	# Install latest version since Ubuntu 20.04's version is too old
	# (doesn't support vsock)
	install_socat
...
start_local_agent()
{
	...
	[ -x "$agent_binary" ] || make -C "${agent_dir}"

	[ -x "$agent_binary" ] \
		|| die "binary does not exist: $agent_binary"
...
run_agent_ctl()
{
	...
	[ -z "$agent_ctl_path" ] && \
		die "agent control binary does not exist: $agent_ctl_binary_name"
